package J0;

import H0.C0165c;
import H0.InterfaceC0163a;
import android.app.ActivityManager;
import android.content.Context;
import android.graphics.Bitmap;
import android.os.Build;
import com.facebook.imagepipeline.producers.Y;
import java.util.Map;
import java.util.Set;
import kotlin.jvm.internal.DefaultConstructorMarker;

/* JADX INFO: renamed from: J0.u, reason: case insensitive filesystem */
/* JADX INFO: loaded from: classes.dex */
public final class C0186u implements InterfaceC0187v {

    /* JADX INFO: renamed from: K, reason: collision with root package name */
    public static final b f622K = new b(null);

    /* JADX INFO: renamed from: L, reason: collision with root package name */
    private static c f623L = new c();

    /* JADX INFO: renamed from: A, reason: collision with root package name */
    private final Set f624A;

    /* JADX INFO: renamed from: B, reason: collision with root package name */
    private final boolean f625B;

    /* JADX INFO: renamed from: C, reason: collision with root package name */
    private final S.d f626C;

    /* JADX INFO: renamed from: D, reason: collision with root package name */
    private final x f627D;

    /* JADX INFO: renamed from: E, reason: collision with root package name */
    private final boolean f628E;

    /* JADX INFO: renamed from: F, reason: collision with root package name */
    private final L0.a f629F;

    /* JADX INFO: renamed from: G, reason: collision with root package name */
    private final H0.x f630G;

    /* JADX INFO: renamed from: H, reason: collision with root package name */
    private final H0.x f631H;

    /* JADX INFO: renamed from: I, reason: collision with root package name */
    private final InterfaceC0163a f632I;

    /* JADX INFO: renamed from: J, reason: collision with root package name */
    private final Map f633J;

    /* JADX INFO: renamed from: a, reason: collision with root package name */
    private final Bitmap.Config f634a;

    /* JADX INFO: renamed from: b, reason: collision with root package name */
    private final X.n f635b;

    /* JADX INFO: renamed from: c, reason: collision with root package name */
    private final H0.x.a f636c;

    /* JADX INFO: renamed from: d, reason: collision with root package name */
    private final H0.x.a f637d;

    /* JADX INFO: renamed from: e, reason: collision with root package name */
    private final H0.k f638e;

    /* JADX INFO: renamed from: f, reason: collision with root package name */
    private final Context f639f;

    /* JADX INFO: renamed from: g, reason: collision with root package name */
    private final EnumC0180n f640g;

    /* JADX INFO: renamed from: h, reason: collision with root package name */
    private final X.n f641h;

    /* JADX INFO: renamed from: i, reason: collision with root package name */
    private final X.n f642i;

    /* JADX INFO: renamed from: j, reason: collision with root package name */
    private final InterfaceC0182p f643j;

    /* JADX INFO: renamed from: k, reason: collision with root package name */
    private final H0.t f644k;

    /* JADX INFO: renamed from: l, reason: collision with root package name */
    private final M0.c f645l;

    /* JADX INFO: renamed from: m, reason: collision with root package name */
    private final W0.d f646m;

    /* JADX INFO: renamed from: n, reason: collision with root package name */
    private final X.n f647n;

    /* JADX INFO: renamed from: o, reason: collision with root package name */
    private final Integer f648o;

    /* JADX INFO: renamed from: p, reason: collision with root package name */
    private final X.n f649p;

    /* JADX INFO: renamed from: q, reason: collision with root package name */
    private final S.d f650q;

    /* JADX INFO: renamed from: r, reason: collision with root package name */
    private final p001a0.d f651r;

    /* JADX INFO: renamed from: s, reason: collision with root package name */
    private final int f652s;

    /* JADX INFO: renamed from: t, reason: collision with root package name */
    private final Y f653t;

    /* JADX INFO: renamed from: u, reason: collision with root package name */
    private final int f654u;

    /* JADX INFO: renamed from: v, reason: collision with root package name */
    private final G0.b f655v;

    /* JADX INFO: renamed from: w, reason: collision with root package name */
    private final R0.D f656w;

    /* JADX INFO: renamed from: x, reason: collision with root package name */
    private final M0.e f657x;

    /* JADX INFO: renamed from: y, reason: collision with root package name */
    private final Set f658y;

    /* JADX INFO: renamed from: z, reason: collision with root package name */
    private final Set f659z;

    /* JADX INFO: renamed from: J0.u$a */
    public static final class a {

        /* JADX INFO: renamed from: A, reason: collision with root package name */
        private S.d f660A;

        /* JADX INFO: renamed from: B, reason: collision with root package name */
        private InterfaceC0183q f661B;

        /* JADX INFO: renamed from: C, reason: collision with root package name */
        private X.n f662C;

        /* JADX INFO: renamed from: D, reason: collision with root package name */
        private int f663D;

        /* JADX INFO: renamed from: E, reason: collision with root package name */
        private final x.a f664E;

        /* JADX INFO: renamed from: F, reason: collision with root package name */
        private boolean f665F;

        /* JADX INFO: renamed from: G, reason: collision with root package name */
        private L0.a f666G;

        /* JADX INFO: renamed from: H, reason: collision with root package name */
        private H0.x f667H;

        /* JADX INFO: renamed from: I, reason: collision with root package name */
        private H0.x f668I;

        /* JADX INFO: renamed from: J, reason: collision with root package name */
        private InterfaceC0163a f669J;

        /* JADX INFO: renamed from: K, reason: collision with root package name */
        private Map f670K;

        /* JADX INFO: renamed from: a, reason: collision with root package name */
        private Bitmap.Config f671a;

        /* JADX INFO: renamed from: b, reason: collision with root package name */
        private X.n f672b;

        /* JADX INFO: renamed from: c, reason: collision with root package name */
        private H0.x.a f673c;

        /* JADX INFO: renamed from: d, reason: collision with root package name */
        private H0.x.a f674d;

        /* JADX INFO: renamed from: e, reason: collision with root package name */
        private H0.k f675e;

        /* JADX INFO: renamed from: f, reason: collision with root package name */
        private final Context f676f;

        /* JADX INFO: renamed from: g, reason: collision with root package name */
        private EnumC0180n f677g;

        /* JADX INFO: renamed from: h, reason: collision with root package name */
        private X.n f678h;

        /* JADX INFO: renamed from: i, reason: collision with root package name */
        private InterfaceC0182p f679i;

        /* JADX INFO: renamed from: j, reason: collision with root package name */
        private H0.t f680j;

        /* JADX INFO: renamed from: k, reason: collision with root package name */
        private M0.c f681k;

        /* JADX INFO: renamed from: l, reason: collision with root package name */
        private X.n f682l;

        /* JADX INFO: renamed from: m, reason: collision with root package name */
        private W0.d f683m;

        /* JADX INFO: renamed from: n, reason: collision with root package name */
        private Integer f684n;

        /* JADX INFO: renamed from: o, reason: collision with root package name */
        private X.n f685o;

        /* JADX INFO: renamed from: p, reason: collision with root package name */
        private S.d f686p;

        /* JADX INFO: renamed from: q, reason: collision with root package name */
        private p001a0.d f687q;

        /* JADX INFO: renamed from: r, reason: collision with root package name */
        private Integer f688r;

        /* JADX INFO: renamed from: s, reason: collision with root package name */
        private Y f689s;

        /* JADX INFO: renamed from: t, reason: collision with root package name */
        private G0.b f690t;

        /* JADX INFO: renamed from: u, reason: collision with root package name */
        private R0.D f691u;

        /* JADX INFO: renamed from: v, reason: collision with root package name */
        private M0.e f692v;

        /* JADX INFO: renamed from: w, reason: collision with root package name */
        private Set f693w;

        /* JADX INFO: renamed from: x, reason: collision with root package name */
        private Set f694x;

        /* JADX INFO: renamed from: y, reason: collision with root package name */
        private Set f695y;

        /* JADX INFO: renamed from: z, reason: collision with root package name */
        private boolean f696z;

        public a(Context context) {
            D2.h.f(context, "context");
            this.f677g = EnumC0180n.AUTO;
            this.f696z = true;
            this.f663D = -1;
            this.f664E = new x.a(this);
            this.f665F = true;
            this.f666G = new L0.b();
            this.f676f = context;
        }

        public final M0.c A() {
            return this.f681k;
        }

        public final M0.d B() {
            return null;
        }

        public final W0.d C() {
            return this.f683m;
        }

        public final Integer D() {
            return this.f684n;
        }

        public final S.d E() {
            return this.f686p;
        }

        public final Integer F() {
            return this.f688r;
        }

        public final p001a0.d G() {
            return this.f687q;
        }

        public final Y H() {
            return this.f689s;
        }

        public final G0.b I() {
            return this.f690t;
        }

        public final R0.D J() {
            return this.f691u;
        }

        public final M0.e K() {
            return this.f692v;
        }

        public final Set L() {
            return this.f694x;
        }

        public final Set M() {
            return this.f693w;
        }

        public final boolean N() {
            return this.f696z;
        }

        public final V.d O() {
            return null;
        }

        public final S.d P() {
            return this.f660A;
        }

        public final X.n Q() {
            return this.f685o;
        }

        public final a R(EnumC0180n enumC0180n) {
            D2.h.f(enumC0180n, "downsampleMode");
            this.f677g = enumC0180n;
            return this;
        }

        public final a S(Y y3) {
            this.f689s = y3;
            return this;
        }

        public final a T(Set set) {
            this.f693w = set;
            return this;
        }

        public final C0186u a() {
            return new C0186u(this, null);
        }

        public final x.a b() {
            return this.f664E;
        }

        public final Bitmap.Config c() {
            return this.f671a;
        }

        public final H0.x d() {
            return this.f667H;
        }

        public final H0.n.b e() {
            return null;
        }

        public final InterfaceC0163a f() {
            return this.f669J;
        }

        public final X.n g() {
            return this.f672b;
        }

        public final H0.x.a h() {
            return this.f673c;
        }

        public final H0.k i() {
            return this.f675e;
        }

        public final T.a j() {
            return null;
        }

        public final L0.a k() {
            return this.f666G;
        }

        public final Context l() {
            return this.f676f;
        }

        public final Set m() {
            return this.f695y;
        }

        public final boolean n() {
            return this.f665F;
        }

        public final X.n o() {
            return this.f662C;
        }

        public final EnumC0180n p() {
            return this.f677g;
        }

        public final Map q() {
            return this.f670K;
        }

        public final X.n r() {
            return this.f682l;
        }

        public final H0.x s() {
            return this.f668I;
        }

        public final X.n t() {
            return this.f678h;
        }

        public final H0.x.a u() {
            return this.f674d;
        }

        public final InterfaceC0182p v() {
            return this.f679i;
        }

        public final x.a w() {
            return this.f664E;
        }

        public final InterfaceC0183q x() {
            return this.f661B;
        }

        public final int y() {
            return this.f663D;
        }

        public final H0.t z() {
            return this.f680j;
        }
    }

    /* JADX INFO: renamed from: J0.u$b */
    public static final class b {
        public /* synthetic */ b(DefaultConstructorMarker defaultConstructorMarker) {
            this();
        }

        /* JADX INFO: Access modifiers changed from: private */
        public final S.d e(Context context) {
            S.d dVarN;
            if (V0.b.d()) {
                V0.b.a("DiskCacheConfig.getDefaultMainDiskCacheConfig");
                try {
                    dVarN = S.d.m(context).n();
                    V0.b.b();
                } catch (Throwable th) {
                    V0.b.b();
                    throw th;
                }
            } else {
                dVarN = S.d.m(context).n();
            }
            D2.h.e(dVarN, "traceSection(...)");
            return dVarN;
        }

        /* JADX INFO: Access modifiers changed from: private */
        public final W0.d f(a aVar) {
            if (aVar.C() == null || aVar.D() == null) {
                return aVar.C();
            }
            throw new IllegalStateException("You can't define a custom ImageTranscoderFactory and provide an ImageTranscoderType");
        }

        /* JADX INFO: Access modifiers changed from: private */
        public final int g(a aVar, x xVar) {
            Integer numF = aVar.F();
            if (numF != null) {
                return numF.intValue();
            }
            if (xVar.n() == 2 && Build.VERSION.SDK_INT >= 27) {
                return 2;
            }
            if (xVar.n() == 1) {
                return 1;
            }
            xVar.n();
            return 0;
        }

        public final c d() {
            return C0186u.f623L;
        }

        public final a h(Context context) {
            D2.h.f(context, "context");
            return new a(context);
        }

        private b() {
        }
    }

    /* JADX INFO: renamed from: J0.u$c */
    public static final class c {

        /* JADX INFO: renamed from: a, reason: collision with root package name */
        private boolean f697a;

        public final boolean a() {
            return this.f697a;
        }
    }

    public /* synthetic */ C0186u(a aVar, DefaultConstructorMarker defaultConstructorMarker) {
        this(aVar);
    }

    public static final a L(Context context) {
        return f622K.h(context);
    }

    public static final c b() {
        return f622K.d();
    }

    @Override // J0.InterfaceC0187v
    public H0.k A() {
        return this.f638e;
    }

    @Override // J0.InterfaceC0187v
    public EnumC0180n B() {
        return this.f640g;
    }

    @Override // J0.InterfaceC0187v
    public T.a C() {
        return null;
    }

    @Override // J0.InterfaceC0187v
    public X.n D() {
        return this.f635b;
    }

    @Override // J0.InterfaceC0187v
    public M0.c E() {
        return this.f645l;
    }

    @Override // J0.InterfaceC0187v
    public boolean F() {
        return this.f625B;
    }

    @Override // J0.InterfaceC0187v
    public x G() {
        return this.f627D;
    }

    @Override // J0.InterfaceC0187v
    public X.n H() {
        return this.f642i;
    }

    @Override // J0.InterfaceC0187v
    public InterfaceC0182p I() {
        return this.f643j;
    }

    @Override // J0.InterfaceC0187v
    public H0.x.a J() {
        return this.f636c;
    }

    @Override // J0.InterfaceC0187v
    public Set K() {
        return this.f624A;
    }

    @Override // J0.InterfaceC0187v
    public Context c() {
        return this.f639f;
    }

    @Override // J0.InterfaceC0187v
    public R0.D d() {
        return this.f656w;
    }

    @Override // J0.InterfaceC0187v
    public M0.e e() {
        return this.f657x;
    }

    @Override // J0.InterfaceC0187v
    public Map f() {
        return this.f633J;
    }

    @Override // J0.InterfaceC0187v
    public S.d g() {
        return this.f626C;
    }

    @Override // J0.InterfaceC0187v
    public H0.t h() {
        return this.f644k;
    }

    @Override // J0.InterfaceC0187v
    public Set i() {
        return this.f659z;
    }

    @Override // J0.InterfaceC0187v
    public int j() {
        return this.f652s;
    }

    @Override // J0.InterfaceC0187v
    public X.n k() {
        return this.f641h;
    }

    @Override // J0.InterfaceC0187v
    public H0.n.b l() {
        return null;
    }

    @Override // J0.InterfaceC0187v
    public X.n m() {
        return this.f649p;
    }

    @Override // J0.InterfaceC0187v
    public V.d n() {
        return null;
    }

    @Override // J0.InterfaceC0187v
    public L0.a o() {
        return this.f629F;
    }

    @Override // J0.InterfaceC0187v
    public InterfaceC0163a p() {
        return this.f632I;
    }

    @Override // J0.InterfaceC0187v
    public Y q() {
        return this.f653t;
    }

    @Override // J0.InterfaceC0187v
    public H0.x r() {
        return this.f631H;
    }

    @Override // J0.InterfaceC0187v
    public Integer s() {
        return this.f648o;
    }

    @Override // J0.InterfaceC0187v
    public S.d t() {
        return this.f650q;
    }

    @Override // J0.InterfaceC0187v
    public Set u() {
        return this.f658y;
    }

    @Override // J0.InterfaceC0187v
    public W0.d v() {
        return this.f646m;
    }

    @Override // J0.InterfaceC0187v
    public p001a0.d w() {
        return this.f651r;
    }

    @Override // J0.InterfaceC0187v
    public M0.d x() {
        return null;
    }

    @Override // J0.InterfaceC0187v
    public boolean y() {
        return this.f628E;
    }

    @Override // J0.InterfaceC0187v
    public H0.x.a z() {
        return this.f637d;
    }

    private C0186u(a aVar) {
        Y yH;
        if (V0.b.d()) {
            V0.b.a("ImagePipelineConfig()");
        }
        this.f627D = aVar.w().c();
        X.n nVarG = aVar.g();
        if (nVarG == null) {
            Object systemService = aVar.l().getSystemService("activity");
            if (systemService == null) {
                throw new IllegalStateException("Required value was null.");
            }
            D2.h.d(systemService, "null cannot be cast to non-null type android.app.ActivityManager");
            nVarG = new H0.o((ActivityManager) systemService);
        }
        this.f635b = nVarG;
        H0.x.a aVarH = aVar.h();
        this.f636c = aVarH == null ? new C0165c() : aVarH;
        H0.x.a aVarU = aVar.u();
        this.f637d = aVarU == null ? new H0.A() : aVarU;
        aVar.e();
        Bitmap.Config configC = aVar.c();
        this.f634a = configC == null ? Bitmap.Config.ARGB_8888 : configC;
        H0.k kVarI = aVar.i();
        if (kVarI == null) {
            kVarI = H0.p.f();
            D2.h.e(kVarI, "getInstance(...)");
        }
        this.f638e = kVarI;
        Context contextL = aVar.l();
        if (contextL == null) {
            throw new IllegalStateException("Required value was null.");
        }
        this.f639f = contextL;
        this.f640g = aVar.p();
        X.n nVarT = aVar.t();
        this.f642i = nVarT == null ? new H0.q() : nVarT;
        H0.t tVarZ = aVar.z();
        if (tVarZ == null) {
            tVarZ = H0.B.o();
            D2.h.e(tVarZ, "getInstance(...)");
        }
        this.f644k = tVarZ;
        this.f645l = aVar.A();
        X.n nVarR = aVar.r();
        if (nVarR == null) {
            nVarR = X.o.f2743b;
            D2.h.e(nVarR, "BOOLEAN_FALSE");
        }
        this.f647n = nVarR;
        b bVar = f622K;
        this.f646m = bVar.f(aVar);
        this.f648o = aVar.D();
        X.n nVarQ = aVar.Q();
        if (nVarQ == null) {
            nVarQ = X.o.f2742a;
            D2.h.e(nVarQ, "BOOLEAN_TRUE");
        }
        this.f649p = nVarQ;
        S.d dVarE = aVar.E();
        this.f650q = dVarE == null ? bVar.e(aVar.l()) : dVarE;
        p001a0.d dVarG = aVar.G();
        if (dVarG == null) {
            dVarG = p001a0.e.b();
            D2.h.e(dVarG, "getInstance(...)");
        }
        this.f651r = dVarG;
        this.f652s = bVar.g(aVar, G());
        int iY = aVar.y() < 0 ? 30000 : aVar.y();
        this.f654u = iY;
        if (V0.b.d()) {
            V0.b.a("ImagePipelineConfig->mNetworkFetcher");
            try {
                yH = aVar.H();
                yH = yH == null ? new com.facebook.imagepipeline.producers.E(iY) : yH;
                V0.b.b();
            } catch (Throwable th) {
                V0.b.b();
                throw th;
            }
        } else {
            yH = aVar.H();
            if (yH == null) {
                yH = new com.facebook.imagepipeline.producers.E(iY);
            }
        }
        this.f653t = yH;
        this.f655v = aVar.I();
        R0.D dJ = aVar.J();
        this.f656w = dJ == null ? new R0.D(R0.B.n().m()) : dJ;
        M0.e eVarK = aVar.K();
        this.f657x = eVarK == null ? new M0.g() : eVarK;
        Set setM = aVar.M();
        this.f658y = setM == null ? p075s2.L.b() : setM;
        Set setL = aVar.L();
        this.f659z = setL == null ? p075s2.L.b() : setL;
        Set setM2 = aVar.m();
        this.f624A = setM2 == null ? p075s2.L.b() : setM2;
        this.f625B = aVar.N();
        S.d dVarP = aVar.P();
        this.f626C = dVarP == null ? t() : dVarP;
        aVar.B();
        int iD = d().d();
        InterfaceC0182p interfaceC0182pV = aVar.v();
        this.f643j = interfaceC0182pV == null ? new C0168b(iD) : interfaceC0182pV;
        this.f628E = aVar.n();
        aVar.j();
        this.f629F = aVar.k();
        this.f630G = aVar.d();
        InterfaceC0163a interfaceC0163aF = aVar.f();
        this.f632I = interfaceC0163aF == null ? new H0.l() : interfaceC0163aF;
        this.f631H = aVar.s();
        aVar.O();
        this.f633J = aVar.q();
        X.n nVarO = aVar.o();
        if (nVarO == null) {
            InterfaceC0183q interfaceC0183qX = aVar.x();
            nVarO = new C0177k(interfaceC0183qX == null ? new C0178l(new C0181o()) : interfaceC0183qX, this);
        }
        this.f641h = nVarO;
        G().y();
        if (V0.b.d()) {
            V0.b.b();
        }
    }
}
